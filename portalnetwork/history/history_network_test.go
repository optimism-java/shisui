...
func TestContentKey(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run(c.name, func(t *testing.T) {
			...
			contentId := ContentId([]byte(contentKey))
...
func TestValidateEpochAccu(t *testing.T) {
	...
	require.NoError(t, err)
	epochAccuBytes, err := getEpochAccuBytes("0x5ec1…4218")
	require.NoError(t, err)
	epochAccu, err := getEpochAccu("0x5ec1…4218")
	require.NoError(t, err)
...
func (m *MockStorage) Get(contentId []byte) ([]byte, error) {
	...
	return nil, errors.New("content is not exist")
}
